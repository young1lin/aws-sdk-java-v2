...
public class AndExpressionParser implements Parser<AndExpression> {
    private static final Parser<AndExpression> PARSER =
        new TrimWhitespaceParser<>(new CheckBoundsParser<>(new AndExpressionParser()));

    private AndExpressionParser() {
    }

    public static Parser<AndExpression> instance() {
        return PARSER;
    }
    ...
    @Override
    public ParseResult<AndExpression> parse(int startPosition, int endPosition, ParserContext context) {
    }
}
