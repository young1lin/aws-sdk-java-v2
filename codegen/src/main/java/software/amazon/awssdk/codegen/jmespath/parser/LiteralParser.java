...
public class LiteralParser implements Parser<Literal> {
    private static final Parser<Literal> PARSER =
        new TrimWhitespaceParser<>(new CheckBoundsParser<>(new LiteralParser()));

    private LiteralParser() {
    }

    public static Parser<Literal> instance() {
        return PARSER;
    }
    ...
    @Override
    public ParseResult<Literal> parse(int startPosition, int endPosition, ParserContext context) {
    }
}
